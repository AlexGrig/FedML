...
def vert_fl_distributed_old(
    ...
    if process_id == server_rank:
        init_server_old(comm, server_model, process_id, worker_number, device, args)
    else:
        init_client_old(
            comm,
            client_model,
...
def init_client(model, dataset, params):
    pass


def init_server_old(comm, server_model, process_id, worker_number, device, args):
    arg_dict = {
        "comm": comm,
        "model": server_model,
